...
const FilterComponent = () => {
  ...
  return (
    <>
      ...
      <div className={`${isHidden ? "hidden" : "block"} lg:block fixed inset-0 z-50 bg-white p-6 lg:relative lg:p-0 lg:z-0`}>
        {filters.map(filter => (
          <div key={filter.id} className={"flex flex-col border-gray-300 border border-l-transparent border-r-transparent border-b-transparent pt-5 pb-5"}>
            ...
            <ul className={openSection === filter.id ? "flex flex-col gap-2 transition-all duration-300" : "hidden transition-all duration-300"}>
